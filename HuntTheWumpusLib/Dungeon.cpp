    ...
    void Dungeon::MakeMove(const DungeonMove operation, const std::vector<int>& destinationIds)
    {
        ...
        if (operation == DungeonMove::Move)
        {
            ...
            else
            {
                ...
                m_providers.m_notification.Notify(UserNotification::Notification::IllegalMoves);
            }
        }
        ...
        if (operation == DungeonMove::Shoot)
        {
            ...
            // Arrow missed (only if game is still running and arrow didn't kill anything)
            if (m_providers.m_change.IsPlaying())
            {
                m_providers.m_notification.Notify(UserNotification::Notification::ArrowMissed);
            }

            // Wumpus moves if game is still playing
            if (m_providers.m_change.IsPlaying())
            {
            ...
            if (m_providers.m_change.IsPlaying() && hunter->OutOfArrows())
            {
                ...
                m_providers.m_notification.Notify(UserNotification::Notification::OutOfArrows);
