...
namespace HuntTheWumpus
{
    ...
    void Cave::HunterEnteredCave(HuntTheWumpus::UserNotification& notifier)
    {
        // 1. Notify that the hunter entered this cave
        notifier.Notify(UserNotification::Notification::HunterEnteredCave);

        // 2. Check all connected caves for nearby denizens
        for (const auto& [id, connectedCave] : m_tunnels)
        {
            const auto cavePtr = connectedCave.lock();
            if (!cavePtr)
            {
                continue;
            }

            // Check each denizen category
            if (cavePtr->HasDenizen({ HuntTheWumpus::Category::Wumpus }))
            {
                notifier.Notify(UserNotification::Notification::NearbyWumpus);
            }

            if (cavePtr->HasDenizen({ HuntTheWumpus::Category::Pit }))
            {
                notifier.Notify(UserNotification::Notification::NearbyPit);
            }

            if (cavePtr->HasDenizen({ HuntTheWumpus::Category::Bat }))
            {
                notifier.Notify(UserNotification::Notification::NearbyBat);
            }
        }
    }
}
